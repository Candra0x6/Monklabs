 ...
 * 
 * Interactive NFT/asset card with spinning animation on hover.
 * Displays a scrolling strip of asset images that spins randomly when hovered.
 * 
 * Features:
 * - Virtual scrolling optimization (10 unique items, 100 total in strip)
 * - Smooth spinning animation using GSAP (2 second duration)
 * - Corner marker decorations
 * - Responsive sizing based on grid
 * 
 * Animation Logic:
 * - Mouse enter triggers random spin (5-9 items)
 * - Seamless looping with virtual list reset
 * - Uses willChange optimization for smooth 60fps animation
 * 
 * Virtual List:
 * - Only 10 unique assets rendered multiple times
 * - Allows infinite scrolling effect without rendering thousands of elements
 * - Automatically resets position after each spin
 */
...
/**
 * AssetCardProps: Component props interface
 */
interface AssetCardProps {
  /** Asset data object (optional, derived from index if not provided) */
  asset?: Asset;
  /** Additional CSS classes to apply to the card */
  className?: string;
  /** Index position in grid (used to select initial asset) */
  index?: number;
}

/**
 * AssetCard: Interactive spinning asset card
 * 
 * Displays an asset image that changes on hover with smooth spinning animation.
